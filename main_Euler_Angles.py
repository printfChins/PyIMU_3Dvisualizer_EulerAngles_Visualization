...
def DrawGL():
    ...
    DrawText("Yaw: {}°   Pitch: {}°   Roll: {}°".format(round(myimu.Yaw,1), round(myimu.Pitch,1), round(myimu.Roll,1)))
    DrawBoard()
    pygame.display.flip()
...
def ReadData():
    while True:
        ...
        if serial_input:
            ...
            if len(parts) == 3:
                try:
                    myimu.Yaw = float(parts[0])
                    myimu.Pitch = float(parts[1])
                    myimu.Roll = float(parts[2])
                except ValueError:
                    pass
